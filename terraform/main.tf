...
resource "azurerm_private_dns_a_record" "app_record" {
  ...
  ttl                 = 300
  records             = [
    azurerm_private_endpoint.app_pe.private_service_connection[0].private_ip_address
  ]
}
...
data "azurerm_key_vault_secret" "appgw_ssl_secret" {
...
resource "azurerm_application_gateway" "appgw" {
  ...
  ssl_certificate {
    name     = "appgw-ssl-cert"
    key_vault_secret_id = data.azurerm_key_vault_secret.appgw_ssl_secret.id
  }
